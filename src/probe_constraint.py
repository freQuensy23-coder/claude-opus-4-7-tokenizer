"""Probe constraint-miner candidates."""
...
async def main():
    ...
    with open("/tmp/constraint_cands.txt") as f:
        cands = [line.rstrip("\n") for line in f if line.strip()]
    print(f"loaded {len(cands)} candidates")

    out = Path("state") / "phase_constraint"
    async with SandwichCounter(cfg) as c:
        await batch_probe(c, cands, out, desc="constraint", progress_every=500)
